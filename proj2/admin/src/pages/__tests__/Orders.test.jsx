...
const mockOrders = [
  {
    _id: 'order1',
    items: [{ name: 'Food 1', quantity: 2 }, { name: 'Food 2', quantity: 1 }],
    address: {
      firstName: 'John',
      lastName: 'Doe',
      street: '123 Main St',
...
describe('Orders Page (Admin)', () => {
  ...
  it('should show empty state when no orders', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No current orders right now/)).toBeInTheDocument();
    });
  });
  ...
  it('should show error on fetch failure', async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Network error while fetching orders');
    });
  });
